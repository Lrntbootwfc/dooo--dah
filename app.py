...
def init_db():
    ...
    # ① Users — now stores permanent global alignments as JSON blob
    cursor.execute('''
...
def commit_matrix_state():
    ...
    {
      "username":           "alice",
      "fs_tree":            [ ... ],       // full tree; each file node may carry a
                                           // "characters": [...] list (per-entry chars)
      "avatar_desc":        "...",         // Self alignment description
      "global_alignments": {              // Permanent baseline alignments
          "self":   "...",
          "father": "...",
          "mother": "...",
          "others": "..."
    ...
    The route does an UPSERT so data is NEVER lost between sessions.
    It is deleted only via /api/delete-account.
    """
    data     = request.json or {}
...
def update_entry_characters():
    """
    Saves the characters array for a specific file entry without requiring
    the frontend to send the entire fs_tree.  Handy for lightweight updates.

    Body: { "username": "alice", "file_id": "file_abc123",
...
def _patch_file_characters(tree: list, target_id: str, characters: list) -> bool:
    """Recursively walks the fs_tree and patches characters on the matching file."""
    for node in tree:
...
def get_global_alignments():
    """Return the permanent character alignments for a user."""
...
def update_global_alignments():
    """
    Save permanent baseline alignments (Self, Father, Mother, Others).
    Body: { "username": "alice", "avatar_desc": "...",
            "global_alignments": { "self":"..","father":"..","mother":"..","others":".." } }
    """
    data     = request.json or {}
...
# ═══════════════════════════════════════════════════════════════════════════════
#  COMIC GENERATION  –  now merges entry content + per-entry chars + global aligns
# ═══════════════════════════════════════════════════════════════════════════════
...
def process_cached_artwork_panel():
    ...
    Body:
    {
      ...
      "content":            "...",          // entry text
      "mood":               "😊",
      "image_seed":         "data:...",     // optional doodle
      "entry_characters":   [...],          // per-entry characters (from currentFile.characters)
      "avatar_desc":        "...",          // Self (permanent)
      "global_alignments": { "father":"..","mother":"..","others":"..." }
    }
    """
    data     = request.json or {}
    username = data.get("username")
    ...
    entry_characters   = data.get("entry_characters", [])   # per-entry chars
    avatar_desc        = data.get("avatar_desc", "")
    global_alignments  = data.get("global_alignments", {})
    ...
    # Pull permanent alignments from DB as fallback if not sent by client
    ...
    # ── Build enriched character context ────────────────────────────────────
    ...
    try:
        ...
        print(f"[Pollinations] status {resp.status_code} – using SVG fallback")
    except Exception as e:
...
def _build_svg_fallback(mood, content, avatar_desc, father_desc, mother_desc,
  ...
  <g transform="translate(15,15)">
    ...
    <text x="70" y="58" font-family="sans-serif" font-size="8.5" fill="#475569">{safe_avatar[26:52]}</text>
  </g>
  <!-- Panel 2: Family alignments -->
  <g transform="translate(15,110)">
    ...
    {father_block}
    {mother_block}
    {'<text x="100" y="210" font-family="sans-serif" font-size="8.5" text-anchor="middle" fill="#475569">' + safe_others[:30] + '</text>' if safe_others else ''}
  </g>
  <!-- Panel 3: Entry characters -->
  ...
  <g transform="translate(15,325)">
    ...
    <text x="20" y="65" font-family="monospace" font-size="10" fill="{theme['text']}">{safe_content[60:120]}</text>
    <text x="20" y="80" font-family="monospace" font-size="10" fill="{theme['text']}">{safe_content[120:150]}</text>
  </g>
  <!-- Footer -->
...
def _build_initial_seed():
    return [{
        ...
        "children": [{
            ...
            "content": "Welcome to my encrypted Comic Diary! Today I am sketching "
                       "out ideas for our first graphic novel.",
            "mood": "😊",
            "created": "6/27/2026, 9:00 AM",
            "edited":  "6/27/2026, 9:00 AM",
            "comic": "",
            "stickers": [],
            "characters": []    # per-entry character list starts empty
        }]
    }]
